...
public class CustomWorldGenBigMushroom extends WorldGenBigMushroom {
	
	private final Block mushroomBlock;
	private final boolean rounded;
	
	public CustomWorldGenBigMushroom(Block mushroomBlock, boolean rounded) {
		super(mushroomBlock);
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		Block block = this.mushroomBlock;
		...
		if(position.getY() >= 1 && position.getY() + i + 1 < 256) {
			...
			if(!flag) return false;
				...
				if(block1 == Blocks.DIRT || block1 == Blocks.GRASS || block1 == Blocks.MYCELIUM ||
						(block1 instanceof ICustomBlockFertile &&
								(((ICustomBlockFertile)block1).getSupportsGeneralPlants() ||
								((ICustomBlockFertile)block1).getSupportsMushrooms()))) {
					int k2 = position.getY() + i;
					
					if(this.rounded || block == Blocks.RED_MUSHROOM_BLOCK) {
					...
					for(int l2 = k2; l2 <= position.getY() + i; ++l2) {
						...
						if(l2 < position.getY() + i) ++j3;
						
						if(!this.rounded || block == Blocks.BROWN_MUSHROOM_BLOCK) j3 = 3;
						...
						for(int l1 = k3; l1 <= l3; ++l1) {
							for(int i2 = j1; i2 <= k1; ++i2) {
								...
								if(!this.rounded || block == Blocks.BROWN_MUSHROOM_BLOCK || l2 < position.getY() + i) {
