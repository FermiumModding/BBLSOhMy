...
public class CustomItemBoat extends ItemBoat {
	
	private final CustomEntityBoat.CustomBoatType customBoatType;
	
	public CustomItemBoat(String name, Item plankItem, int plankMeta, Item stickItem, int stickMeta) {
		super(EntityBoat.Type.OAK);
		this.setRegistryName(BBLSOhMy.MODID + ":" + name);
		this.setTranslationKey(BBLSOhMy.MODID + "." + name);
		CustomEntityBoat.CustomBoatType customType = new CustomEntityBoat.CustomBoatType(name);
		customType.setItemBoat(this);
		customType.setDroppedPlanks(plankItem, plankMeta);
		customType.setDroppedSticks(stickItem, stickMeta);
		CustomEntityBoat.CustomBoatType.CUSTOM_BOAT_TYPES.add(customType);
		this.customBoatType = customType;
	}
	
	public CustomEntityBoat.CustomBoatType getCustomBoatType() {
		return this.customBoatType;
	}
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		...
		if(raytraceresult == null) {
			...
			if(flag) {
				...
				CustomEntityBoat entityboat = new CustomEntityBoat(worldIn, raytraceresult.hitVec.x, flag1 ? raytraceresult.hitVec.y - 0.12D : raytraceresult.hitVec.y, raytraceresult.hitVec.z);
				entityboat.setCustomBoatType(this.customBoatType);
